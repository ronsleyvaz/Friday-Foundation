...
4. Register it in `PACK_COMMANDS` in `install.sh`. The catalog parity tests then require it in every other place a command is listed (the README and manual tables, the installer's output, the command counts, and `new-capability.md`'s reserved-name list) and name any you miss. Add the name to `COMMANDS` in `friday-usage.sh` as well, with a folder form in `classify_output` if it writes into its own folder.
